...
async function getWeatherDetails(cityName = 'Delhi') {
    try {
        ...
        console.log(city)
        let url = `https://api.open-meteo.com/v1/forecast?...& // line break here
timezone=auto`;
        url = url.replace('...', `latitude=${latitude}&longitude=${longitude}&current_weather=true&hourly=relative_humidity_2m`);
        console.log(url);
